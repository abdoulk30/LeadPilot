...
to put it, and event_id is the only handle execute() is given.
...
class WriteExecutionFailedAfterApprovalError(Exception):
    ...
    write itself raised. The approval was consumed — a retry needs a
    fresh run()/approve() cycle, not another call to this event_id.
    """


def _encode_content_ref(source_id: str, row_ref: str, field: str, value: str) -> str:
    return json.dumps({"source_id": source_id, "row_ref": row_ref, "field": field, "value": value})
...
def run(
    ...
    try:
        ...
        event = gate.create_draft(
            ...
            content_ref=_encode_content_ref(source_id, row_ref, field, value),
        )
        session.commit()
        return {
...
def execute(session: Session, event_id: uuid.UUID, connector: LeadSourceConnector | None = None) -> dict:
    ...
    connector = connector or GoogleSheetsConnector(session, rep_id)
    try:
        connector.commit_field_write(info["source_id"], info["row_ref"], info["field"], info["value"])
