...
def test_full_lifecycle(db_session):
    ...
    assert gate.approve(db_session, event.event_id, rep_id="rep-1") is True
    db_session.refresh(event)
    assert event.stage == Stage.APPROVED
    assert event.rep_id == "rep-1"
...
def test_cannot_execute_twice(db_session):
    ...
    gate.approve(db_session, event.event_id, rep_id="rep-1")
...
def test_cannot_approve_an_already_approved_event(db_session):
    ...
    assert gate.approve(db_session, event.event_id, rep_id="rep-2") is False
...
def test_reject_then_cannot_approve_or_execute(db_session):
    ...
    assert gate.reject(db_session, event.event_id, rep_id="rep-1") is True
    db_session.refresh(event)
    assert event.stage == Stage.REJECTED

    assert gate.approve(db_session, event.event_id, rep_id="rep-1") is False
    assert gate.try_execute(db_session, event.event_id) is False
...
def test_try_execute_is_single_use_under_concurrency():
    ...
    gate.approve(setup, event.event_id, rep_id="rep-1")
    setup.commit()
